...
// function.
var warnForkAfterConsume = node(function () {
...
// Hack our way around the fact that util.deprecate is all-or-nothing for a
// function.
var warnMapWithValue = node(function() {
 ...
 * @return
 */
var errorsTo = function errorsTo(stream) {
  var errorStream = stream.observe().filter(function() { return false; });

  stream = stream.errors(function() {});
...
exports.through = function through(process) {
  return function(stream) {
    ...
    errorsToResults = highlandErrorsTo(stream);
...
var process_2 = process_1.through;

var highlandProcess = process_1;
...
const parseCommandArgs = (args = {}) => {
  return Object.entries(args).reduce((acc, curr) => {
    ...
    const modifiedKey = `--${lodash.kebabCase(key)}`;

    if (Array.isArray(value)) {
      return acc.concat(value.reduce((acc, curr) => {
...
class TabulaCommand {
  ...
  _build(pdfPath, commandArgs) {
    const silentArgs = commandArgs && commandArgs.silent ? ['-Dorg.slf4j.simpleLogger.defaultLogLevel=off', '-Dorg.apache.commons.logging.Log=org.apache.commons.logging.impl.NoOpLog', '-Dfile.encoding=utf-8'] : [];
    this._args = [...silentArgs, ...['-jar', JAR_PATH], ...parseCommandArgs(commandArgs), pdfPath];
  }
...
var Command = TabulaCommand;
var parseCommandArgs_1 = parseCommandArgs;
Command.parseCommandArgs = parseCommandArgs_1;
...
class Tabula {
  ...
  stream() {
    return highlandProcess.from(new Command(this._pdfPath, this._options).run());
  }

  streamSections(callback) {
    this.stream().map(data => data.toString()).split().collect().stopOnError(err => callback(err, null)).each(data => callback(null, data));
  }
  ...
  getData() {
    ...
    const {
      stdout,
      stderr
    } = result; // TODO Check status/signal/error
